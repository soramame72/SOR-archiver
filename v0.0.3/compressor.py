...
class SORCompressor:
    ...
    def compress_single_file_parallel(self, file_info: Tuple[str, str, str]) -> Tuple[str, bytes, int, int, int]:
        """
        単一ファイルを圧縮（並列処理用）
    ...
    def compress_to_sor_parallel(self, file_paths: List[str], output_path: str, root_dir: str = None, progress_callback=None) -> Dict:
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            # 各ファイルの圧縮を並列実行
            future_to_file = {executor.submit(self.compress_single_file_parallel, file_info): file_info for file_info in file_infos}
        ...
        with open(output_path, 'wb') as f:
            ...
            for relative_path, compressed_data, method_code, file_type, original_size in compressed_files:
                ...
                if method_code == METHOD_DUP_REF:
                    # 重複参照の場合
                    ref_index = self.hash_map[hashlib.sha256(open(os.path.join(root_dir, relative_path) if root_dir else relative_path, 'rb').read()).digest()]
                    f.write(struct.pack('<I', ref_index))
                else:
                    # 圧縮データサイズを必ず書き込む
                    f.write(struct.pack('<I', int(len(compressed_data))))
...
def compress_to_sor_parallel(file_paths, output_path, root_dir=None, progress_callback=None, max_workers=None):
    ...
    Returns:
        Dict: 圧縮統計情報
    """
    compressor = SORCompressor(max_workers=max_workers)
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        ...
        print('Usage: python compressor.py --parallel output.sor input1 [input2 ...]')
        sys.exit(1)
    ...
    if use_parallel:
        print("並列処理モードで実行")
        compressor = SORCompressor(max_workers=max_workers)
        stats = compressor.compress_to_sor_parallel(input_files, output_path)
    else:
        print("逐次処理モードで実行")
        compressor = SORCompressor()
        stats = compressor.compress_to_sor(input_files, output_path)
    ...
    if 'parallel_workers' in stats:
        print(f"使用ワーカー数: {stats['parallel_workers']}")
