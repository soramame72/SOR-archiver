...
class SORApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title('SOR Archiver for MacOS')
    ...
    def create_widgets(self):
        ...
        logo_label = tk.Label(self.left_frame, text='SOR Archiver for MacOS', font=('Segoe UI', 20, 'bold'), fg='#2563eb', bg='#e5e5e5')
    ...
    def create_compress_ui(self):
        ...
        self.compress_widgets.append(path_frame)
        ...
        self.compress_widgets.append(out_frame)
        ...
        self.progress1.pack(pady=(10,18))
    ...
    def create_decompress_ui(self):
        ...
        self.progress2.pack(pady=(10,18))
    ...
    def show_mode(self, mode):
        ...
        if mode == 'compress':
            for w in self.compress_widgets:
                if w == self.compress_cancel_btn: continue
                w.pack()
        else:
            for w in self.decompress_widgets:
                if w == self.decompress_cancel_btn: continue
                w.pack()
    ...
    def start_compress(self):
        ...
        if not folder or not out_path:
            messagebox.showerror('エラー', 'フォルダと出力先を指定してください')
            return
        ...
        file_list = [os.path.relpath(os.path.join(r,f), folder).replace("\\","/") for r,d,fs in os.walk(folder) for f in fs]
    ...
    def start_decompress(self):
        ...
        if not sor_path or not out_dir:
            messagebox.showerror('エラー', 'アーカイブと出力先を指定してください')
            return
        now = datetime.datetime.now()
        dir_name = f'sor_{now:%Y-%m-%d-%H-%M-%S}'
    ...
    # ---------------- キャンセルボタン ----------------
    def cancel_compress(self): self._compress_cancelled = True; self.compress_status.config(text='キャンセル中...')
    def cancel_decompress(self): self._decompress_cancelled = True; self.decompress_status.config(text='キャンセル中...')
    def show_compress_cancel(self, show=True):
        if show: self.compress_cancel_btn.pack(pady=(15,5))
        else: self.compress_cancel_btn.pack_forget()
    def show_decompress_cancel(self, show=True):
        if show: self.decompress_cancel_btn.pack(pady=(15,5))
        else: self.decompress_cancel_btn.pack_forget()
...
4. 「解凍開始」をクリック"""
        ...
        tk.Button(win, text='閉じる', command=win.destroy, font=('Segoe UI', 12), width=16).pack(pady=10)

    # ---------------- 情報 ----------------
    def show_info(self):
        info_text = """製作者: soramame72
Webサイト: http://mamechosu.s323.xrea.com/software/sor/index.html
Version: 0.0.3
...
• 複数の圧縮アルゴリズム対応"""
        ...
        # URL部分を青字でクリック可能
        url = 'http://mamechosu.s323.xrea.com/software/sor/index.html'
        start_idx = '1.0'
        while True:
